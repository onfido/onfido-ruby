  ...
  class UsDrivingLicenceBreakdownAddressBreakdown
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Onfido::UsDrivingLicenceBreakdownAddressBreakdown` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Onfido::UsDrivingLicenceBreakdownAddressBreakdown`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'zip4')
        self.zip4 = attributes[:'zip4']
      end

      if attributes.key?(:'zip5')
        self.zip5 = attributes[:'zip5']
      end
    end
    ...
    def hash
      [city, line_1, line_2, state_code, zip4, zip5].hash
    end
