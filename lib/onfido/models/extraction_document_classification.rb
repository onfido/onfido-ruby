  ...
  class ExtractionDocumentClassification
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Onfido::ExtractionDocumentClassification` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Onfido::ExtractionDocumentClassification`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      subtype_validator = EnumAttributeValidator.new('String', ["full", "not_full", "provisional"])
    ...
    def subtype=(subtype)
      validator = EnumAttributeValidator.new('String', ["full", "not_full", "provisional"])
    ...
    def hash
      [issuing_country, document_type, issuing_state, subtype, version].hash
    end
